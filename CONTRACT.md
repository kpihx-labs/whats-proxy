...
| `story-post` | `sendMessage("status@broadcast")` | WS | ✅ | post a story (text/image/video/audio); optional `status_jid_list` for audience |
...
| `story-reply` | `sendMessage(author)` | WS | ✅ | reply to a story author (private message) |
| `story-delete` | `sendMessage("status@broadcast", { delete })` | WS | ✅ | delete your own story (fromMe guard + revoke) |
...
| **Merged into an existing action** | 0 | — |
 ...
 | **Result** | **68 total** | `65 ported - 3 dead labels + 6 stories` |
...
|------------|------:|---------|
| **approval: always** | 27 | send-text, send-image, send-video, send-audio, send-document, send-sticker, send-location, send-contact, send-reaction, send-poll, edit-message, delete-message, forward-message, chat-manage (dangerous), chat-star, chat-disappearing, group-create, group-subject, group-description, group-participants, group-leave, group-settings, group-picture, channel-create, channel-manage, channel-update, channel-delete |
...
`rejected`. HITL-required actions: 27 always-approval + 9 conditional = 36 total (of 68).
...
**62/65 `whats-mcp` tools ported (3 dead labels removed) + 1 `send-batch` + 6 story actions = 68 total.** Every tool in `$HOME/Work/AI/MCPs/whats_mcp/src/tools/*.js`
...
| **TOTAL ported** | **62** | **68** | ✅ 3 dead labels removed, 6 new stories, send-batch = **68 total** |
...
| **D8** | HITL scope | 28 always + 9 conditional = 37 of 68 actions | narrower policy would miss send safety |
...
`whats-proxy` (68 RPC actions: 62 ported - 3 dead labels + 9 new), modelled on `tick-proxy` v1.1.0 (52 RPC actions).*
