...
Non-MCP CLI proxy for WhatsApp. Full `whats-mcp` catalog (74 actions) as flat JSON-RPC actions
...
> **Status:** 🟢 **IMPLEMENTED — 74 actions, multi-account, pairing working live.** `CONTRACT.md`
> is the architecture contract. Version 0.6.0.

## Overview

```bash
whats-proxy do <action> [payload|file] [-a phone] [-o path] [-f json|table]   # 74 actions
...
- **Actions must be registered** in `actions/registry.ts` (duplicate detection on boot; registry
  test asserts 74; audit test asserts 74 schemas).
...
62/65 `whats-mcp` tools ported (3 dead labels removed) + 1 `send-batch` + 3 story actions + 13 community actions = **74 actions** total.
