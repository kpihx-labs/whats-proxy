...
console.log("\n[1] catalog help");
{
  ...
  check(`catalog lists ~68 actions (found ${actionCount})`, actionCount >= 60, `count=${actionCount}`);
}
...
console.log("\n[8] store-only actions via CLI (daemon auto-spawn)");
{
  ...
  check("guide lists 68 tools", text.includes("68"), text.match(/"total_tools": (\d+)/)?.[1]);
