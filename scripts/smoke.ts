...
console.log("\n[1] catalog help");
{
  ...
  check(`catalog lists ~76 actions (found ${actionCount})`, actionCount >= 60, `count=${actionCount}`);
}
...
console.log("\n[8] store-only actions via CLI (daemon auto-spawn)");
{
  ...
  check("guide lists 76 tools", text.includes("76"), text.match(/"total_tools": (\d+)/)?.[1]);
